...
export async function getPackages(userid: string, repos: Repo[]) {
  const pkgs = await Promise.all(repos.map(async (repo: Repo) => {
    ...
    const endpoint = `https://raw.githubusercontent.com/${userid}/${repo.name}/${repo.defaultBranch}/requirements.txt`;
    const res = await fetch(endpoint)
    if (res.ok) {
      const requirements = await res.text();
      let packages = requirements.trim().split('\n');
      returnPkgs = packages.map(p => p.split(/[ =<>;]+/)[0])
    }
